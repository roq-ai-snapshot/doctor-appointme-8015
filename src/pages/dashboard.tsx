...
import { HealthcareProviderListPage as ShowCaseList } from 'pages/healthcare-providers';
...
function DashboardPage() {
  ...
  const healthcareProviderStat = useSWR<PaginatedInterface<HealthcareProviderInterface>>(
    () => `/healthcare-providers`,
    () => getHealthcareProviders({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: healthcareProviderStat?.data?.totalCount,
      isLoading: healthcareProviderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Healthcare Providers
                </Text>
                <NextLink href={`healthcare-providers`} passHref legacyBehavior>
                  <Button
